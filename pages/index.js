...
export default function Home() {
  ...
  // Funzione per copiare un testo specifico negli appunti
  ...
  // Funzione per separare le didascalie in un array
  const captionsArray = caption ? caption.split('\n\n') : [];
        ...
        <main className="flex flex-col items-center p-8 max-w-xl w-full bg-white/80 rounded-xl shadow-lg backdrop-blur-sm relative">
          ...
          {captionsArray.length > 0 && (
            <div className="mt-8 p-6 w-full bg-slate-50 border border-slate-200 rounded-lg shadow-inner">
              ...
              {captionsArray.map((singleCaption, index) => (
                <div key={index} className="relative group mb-4 last:mb-0 p-4 border border-slate-300 rounded-lg bg-white">
                  <pre className="whitespace-pre-wrap break-words m-0 p-0 text-base leading-relaxed text-slate-600 pr-10">
                    {singleCaption}
                  </pre>
                  <button
                    onClick={() => handleCopy(singleCaption)}
                    className="absolute top-2 right-2 p-2 rounded-md text-slate-400 opacity-0 group-hover:opacity-100 hover:text-indigo-600 hover:bg-slate-200 transition-all duration-200"
                    title={t.copySuccess}
                  >
          ...
          {copyStatus && (
            <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 bg-slate-800 text-white text-sm px-3 py-1 rounded-full shadow-lg transition-opacity duration-300">
              {copyStatus}
            </div>
          )}
        </main>
