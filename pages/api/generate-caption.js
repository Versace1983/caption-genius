...
Forniscimi solo le didascalie separate da una riga vuota.`;
  ...
  try {
    ...
    // Migliorata la gestione degli errori per risposte non valide o bloccate
    if (result.candidates && result.candidates.length > 0 &&
      ...
      // Il prompt è stato bloccato a causa di problemi di sicurezza
      const safetyRatings = result.promptFeedback.safetyRatings;
      const blockedCategories = safetyRatings.filter(rating => rating.blockReason).map(rating => rating.blockReason);
      const errorMessage = `Il prompt è stato bloccato a causa di contenuti non sicuri. Motivi: ${blockedCategories.join(', ')}.`;
      console.error('Prompt bloccato da Gemini:', errorMessage);
      return res.status(400).json({ error: errorMessage });
    } else {
      // Errore generico per altre risposte inaspettate
